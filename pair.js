...
const config = {
    ...
    BOT_FOOTER: '> 𝙼𝙰𝙳𝙴 𝙱𝚈 𝙽𝙰𝚃𝚂𝚄𝚘𝚛𝙳𝙴𝙽𝚃𝚂𝚄',
...
╰───────────────⭓`;
        ...
        const newsletterContext = {
            ...
            forwardedNewsletterMessageInfo: {
                ...
                newsletterName: '𝙼𝙰𝙳𝙴 𝙱𝚈 𝙽𝙰𝚃𝚂𝚄𝚘𝚛𝙳𝙴𝙽𝚃𝚂𝚄',
                serverMessageId: -1
            }
        };
...
`;
    ...
    const menuMessage = {
      image: { url: "https://files.catbox.moe/s1ggtt.jpeg" },
      caption: `*𝙼𝚄𝙻𝚃𝙸 𝙳𝙴𝙽𝚃𝚂𝚄 𝙼𝙳*\n${menuText}`,
      buttons: [
        {
          buttonId: `${config.PREFIX}quick_commands`,
          buttonText: { displayText: '𝙼𝚄𝙻𝚃𝙸 𝙳𝙴𝙽𝚃𝚂𝚄 𝙼𝙳 ᴄᴍᴅs' },
          type: 4,
          nativeFlowInfo: {
            name: 'single_select',
            paramsJson: JSON.stringify({
              title: '𝙼𝚄𝙻𝚃𝙸 𝙳𝙴𝙽𝚃𝚂𝚄 𝙼𝙳 ᴄᴍᴅs',
              sections: [
                {
                  title: "🌐 ɢᴇɴᴇʀᴀʟ ᴄᴏᴍᴍᴀɴᴅs",
                  highlight_label: '𝙼𝚄𝙻𝚃𝙸 𝙳𝙴𝙽𝚃𝚂𝚄 𝙼𝙳',
                  rows: [
    ...
    break;
                ...
                await socket.sendMessage(sender, { react: { text: '📲', key: msg.key } });
                    ...
                    try {
                        ...
                        await socket.sendMessage(sender, {
                            text: `> *𝙼𝚄𝙻𝚃𝙸 𝙳𝙴𝙽𝚃𝚂𝚄 𝙼𝙳 ᴘᴀɪʀ ᴄᴏᴍᴘʟᴇᴛᴇᴅ* ✅\n\n*🔑 ʏᴏᴜʀ ᴘᴀɪʀɪɴɢ ᴄᴏᴅᴇ ɪs:* ${result.code}`
                        }, { quoted: msg });
...
`;
        ...
        await socket.sendMessage(sender, {
            ...
            contextInfo: {
                ...
                forwardedNewsletterMessageInfo: {
                    newsletterJid: '120363423640959729@newsletter',
                    newsletterName: '𝙼𝚄𝙻𝚃𝙸 𝙳𝙴𝙽𝚃𝚂𝚄 𝙼𝙳',
                    serverMessageId: -1
                }
            }
        }, { quoted: fakevCard });
                ...
                case 'winfo': {
                    ...
                    try {
                        ...
                        if (statusData?.status) {
                            winfoBio = `${statusData.status}\n└─ 📌 ᴜᴘᴅᴀᴛᴇᴅ: ${statusData.setAt ? new Date(statusData.setAt).toLocaleString('en-US', { timeZone: 'America/Port-au-Prince' }) : 'Unknown'}`;
                        }
                    } catch (e) {
                    ...
                    try {
                        ...
                        if (lastSeenData?.lastSeen) {
                            winfoLastSeen = `🕒 ${new Date(lastSeenData.lastSeen).toLocaleString('en-US', { timeZone: 'America/Port-au-Prince' })}`;
                        }
                    } catch (e) {
                ...
                await socket.sendMessage(sender, { react: { text: '😅', key: msg.key } });
                    try {
                        ...
                        await socket.sendMessage(sender, {
                            image: { url: thumbnailUrl },
                            caption: formatMessage(
                                '📰 𝙼𝚄𝙻𝚃𝙸 𝙳𝙴𝙽𝚃𝚂𝚄 𝙼𝙳 ɢᴏssɪᴘ ʟᴀᴛᴇsᴛ ɴᴇᴡs් 📰',
...
await socket.sendMessage(userJid, {
    image: { url: config.RCD_IMAGE_PATH },
    caption: `ᴡᴇʟᴄᴏᴍᴇ ᴛᴏ 𝙼𝚄𝙻𝚃𝙸 𝙳𝙴𝙽𝚃𝚂𝚄 𝙼𝙳
╭───────────────⭓
